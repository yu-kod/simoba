...
### Requirement: Offline mode unchanged
オフラインモードの戦闘エフェクト（CombatManager 経由の直接トリガー）は変更しない。GameMode の新しいコールバック（onAttackEvent, onDamageEvent, onDeathEvent）はオフラインでは no-op。

#### Scenario: Offline melee attack
- **WHEN** オフラインモードでメレー攻撃が発生する
- **THEN** CombatManager が直接 meleeSwing.play() をトリガーする（イベント経由ではない）

#### Scenario: Offline damage
- **WHEN** オフラインモードでダメージが発生する
- **THEN** CombatManager が直接 flash() をトリガーする（イベント経由ではない）
