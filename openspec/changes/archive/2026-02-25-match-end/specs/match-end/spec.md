...
- **THEN** `matchPhase` が `'waiting'`、`winnerTeam` が `''` である
...
- **THEN** `matchPhase` が `'finished'` に変更され、`winnerTeam` に破壊されたタワーの敵チームが設定される
...
### Requirement: 勝敗判定
サーバーは `gameUpdate` の各フレーム末尾で全タワーの `dead` 状態をチェックしなければならない（SHALL）。`dead === true` のタワーが見つかった場合、そのタワーのチームの敵チームを `winnerTeam` に設定し、`matchPhase` を `'finished'` に変更しなければならない（SHALL）。

#### Scenario: blue タワーが破壊される
- **WHEN** blue チームのタワーの `dead` が `true` になる
- **THEN** `winnerTeam` が `'red'`、`matchPhase` が `'finished'` に設定される

#### Scenario: red タワーが破壊される
- **WHEN** red チームのタワーの `dead` が `true` になる
- **THEN** `winnerTeam` が `'blue'`、`matchPhase` が `'finished'` に設定される
