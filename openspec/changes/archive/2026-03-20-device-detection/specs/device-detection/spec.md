...
システムは、Phaser.Device API を利用してデバイス種別（desktop / mobile）とタッチ対応有無を判定する `detectDevice()` 関数を提供しなければならない（SHALL）。
...
タイトル画面（LobbyScene 等）の右下に、判定されたデバイス種別を薄い文字で表示しなければならない（SHALL）。
